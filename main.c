...
int main(void)
{
  ...
  HAL_ADC_Start_DMA(&hadc1, ADC_RawRead, 100);
    ...
    /* USER CODE BEGIN 3 */
	 ...
	 static uint64_t timestamp =0;
	 	  ...
	 	  if(currentTime > timestamp)
	 	  {
	 		  ...
	 		  convert[0]=0x69;
	 		  convert[1]=(uint8_t)(adcavg & 0xFF);
	 		 convert[2]=(uint8_t)((adcavg>>8) & 0xFF);
	 		 convert[3]=0x0A;
	  HAL_UART_Transmit_IT(&hlpuart1,convert,sizeof(convert));
	 	  ...
	 	  if(pwm2>0){
	 		 ...
	 		 __HAL_TIM_SET_COMPARE(&htim4,TIM_CHANNEL_2,pwm2+32676);

	 	  }
	 	  else if(pwm2<0){
	 	 	 		 __HAL_TIM_SET_COMPARE(&htim4,TIM_CHANNEL_1,(-1*pwm2)+32676);
...
void UARTDMAconfig()
{
	...
	pwm2=((pwm-2047)*32676)/2047;
}
void sumadc()
{
	  for(uint16_t i=0;i<100;i++)
	  {
		  sum=sum+ADC_RawRead[i];
	  }
}
