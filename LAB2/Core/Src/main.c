...
/* USER CODE BEGIN PV */
uint16_t ReadADC[2] = {0};
uint16_t RawADC = 0;
uint16_t RawADCmotor = 0;
int position_motor = 0;
float position_poten = 0;
...
HAL_ADC_Start_DMA(&hadc1, ReadADC, 2);
...
{
	RawADC = ReadADC[0];
	RawADCmotor = ReadADC[1];
	position_poten =(RawADC * 360.0)/4095.0;
	position_motor =(RawADCmotor * 360.0)/4095.0;
//	setposition = position_poten;
}
//PWM_control()
//{
//	if( position < (setposition/2))
//		 {
...
//	else
//		 {
//			 __HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_1,control);
//		 }
//}
/* USER CODE END 4 */
